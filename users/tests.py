...
class UsersTest(TestCase):
    ...
    def setUp(self):
        ...
        self.assertEqual(response.data['count'], 0, msg='Список пользователей должен быть пуст')
        ...
        self.assertEqual(response.data['count'], 1, msg='Список пользователей должен быть быть равен 1 так как один так как содали 1 пользователя')
        ...
        self.assertEqual(response.status_code, 201, msg='Должен создаться пользователя 2')
        ...
        self.assertEqual(response.data['count'], 2, msg='Список пользователей должен быть быть равен 2 так как один так как содали 1 пользователя')
        self.assertEqual(response.status_code, 200, msg='Токен работает не коректно')
    ...
    def test_users_list(self):
        ...
        # Проверка количества
        self.assertEqual(response.data['count'], 2, msg='Список пользователей должен быть быть равен 2')
    ...
    def test_subscription(self):
        ...
        self.assertFalse(response.data['is_subscribed'], msg='Должен быть не подписан на ползователя')
        ...
        # Проверка профеля пользователя
        response = self.client.get(f'/api/v1/users/{ID2}/', **self.header1)
        self.assertTrue(response.data['is_subscribed'], msg='Должен быть подписан на ползователя')
